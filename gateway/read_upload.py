...
def cin_url(container_name: str) -> str:
    return f"{CSE_BASE}/~/{CSE_ID}/{CSE_NAME}/{AE_NAME}/{container_name}"
...
def post_cin(container_name: str, con: dict) -> bool:
    payload = {
        "m2m:cin": {
            "con": con
        }
    ...
    try:
        r = requests.post(cin_url(container_name), headers=headers(), json=payload, timeout=5)
    except Exception as e:
        log(f"[post error] cnt={container_name} err={e}")
        return False

    if r.status_code not in (200, 201):
        log(f"[post failed] cnt={container_name} status={r.status_code} body={r.text}")
        return False

    log(f"[post ok] cnt={container_name} con={con}")
    return True

def post_proximity(distance_m: float, tag_id: Optional[str]) -> bool:
    con = {
    ...
    return post_cin(PROXIMITY_CNT, con)


def post_status(link_state: str, tag_id: Optional[str], reason: str) -> bool:
    con = {
        "state": link_state,
    ...
    return post_cin(STATUS_CNT, con)


def post_session(tag_id: Optional[str], event: str) -> bool:
    con = {
        "event": event,
        "tag_id": safe_tag_id(tag_id),
        "tag_name": tag_name_from_id(tag_id),
    ...
    return post_cin(SESSION_CNT, con)
...
def run_once(port: str) -> None:
    ...
    last_link_state = "disconnect"
    last_signal_time = time.time()
    ever_received_signal = False
    ...
    try:
        while True:
            ...
            if not chunk:
                if (
                    ever_received_signal
                    and last_link_state != "disconnect"
                    and now - last_signal_time > SILENCE_TIMEOUT_S
                ):
                    if post_status("disconnect", last_id, "serial_silence"):
                        last_link_state = "disconnect"
                        last_uploaded_distance = None
                        log("[state] disconnect because serial signal stopped")
                continue
            ...
            while b"\n" in buf:
                ...
                if sid is not None:
                    ...
                    if sid != last_session_id:
                        if post_session(last_id, "tag_seen"):
                            last_session_id = sid
                    continue
                ...
                reconnecting = last_link_state != "connected"
                if reconnecting:
                    if post_status("connected", last_id, "distance_seen"):
                        last_link_state = "connected"

                must_upload = reconnecting or should_upload(d, last_uploaded_distance)
